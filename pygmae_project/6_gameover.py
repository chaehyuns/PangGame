...
game_result = "Game Over" # TimeOut(시간초과실패), Mission Complete(성공), GameOver(캐릭터가 공에 맞음)
...
while running: 
    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.KEYUP: #방향키떼면 멈춤
    ...
    #공 위치 정의
    for ball_idx, ball_val in enumerate(balls): #현재 ball index와 값을 출력
        ball_pos_x = ball_val["pos_x"]
        ball_pos_y = ball_val["pos_y"] 
        ball_img_idx = ball_val["img_idx"] #현재 ball의 이미지 좌표를 알 수 있게
        
        ball_size = ball_images[ball_img_idx].get_rect().size
        ball_width = ball_size[0]
        ball_height = ball_size[1]
        
        #가로벽에 닿았을 때 공의 이동 위치 변경(튕겨나오는 효과)
        ...
        for weapon_idx, weapon_val in enumerate(weapons):
            ...
            if weapon_rect.colliderect(ball_rect):
                ...
                if ball_img_idx < 3:
                    ...
                    "pos_y" : ball_pos_y + (ball_height / 2) - (small_ball_height /2), #공의 y좌표
                    "img_idx" : ball_img_idx + 1, #공의 이미지 인덱스
                    "to_x" : 3, #x축 이동방향 +3이면 오른쪽
                    "to_y": -6, #y축 이동 방향,
                    "init_spd_y" : ball_speed_y[ball_img_idx + 1] }) #y 최초 속도   
                break
        else: #계속 게임을 진행
            continue #안쪽 for문 조건이 맞지 않으면 continue
        break #안쪽 for문에서break를 만나면 여기로 진입 가능
...
#게임 오버 메세지
msg = game_font.render(game_result, True, (255,255,0)) #노란색으로 메세지 출력
